...
let checkAnnotation (p: Primer) errorDesc =
    for a in p.annotation do
        if a.il < 0
        then failwithf "primer annotation il (%d) < 0 p=%s %s" a.il (p.Primer.str) errorDesc

        if a.ir >= p.tail.Length + p.body.Length
        then failwithf "primer annotation ir (%d) off end len=%d %s" a.ir (p.tail.Length + p.body.Length) errorDesc

        if a.il > a.ir && p.tail.Length > 0
        then failwithf "primer annotation il(%d) > ir(%d) %s" a.il a.ir errorDesc

        match a.iType with
        | ANNEAL when a.ir - a.il + 1 < 12 ->
            failwithf "annealing region of a primer is less than 12 bases il=%d ir=%d %s" a.il a.ir errorDesc
        | AMP when a.ir - a.il + 1 < 12 ->
            failwithf "amplification region of a primer is less than 12 bases il=%d ir=%d %s" a.il a.ir errorDesc
        | _ -> () // fine
                ...
                | Some (f), Some (r) ->
                    ...
                    if s1 <> s2' then
                        failwithf "primer annotation anneal region fails antiparallel test\nfwd  =%O\nrev  =%O\nrcrev=%O\nname=%s\n"
                            s1 s2 s2' dpp.name
                | None, Some (x) ->
                    failwithf "primer annotation single anneal region rev %d-%d %O" x.il x.ir dpp.rev.Primer
                | Some (x), None ->
                    failwithf "primer annotation single anneal region fwd %d-%d %O" x.il x.ir dpp.fwd.Primer
                | None, None -> () // fine
            | GAP
            | SANDWICHGAP -> ()
    ...
    for pList, assembly in pa do
            ...
            | DPP (dpp) ->
                ...
                if not (assemblySeq.Contains(fwd)) then
                    failwithf "fwd primer validation failure.  Primer %O\ntail=%O\nhead=%O\n does not occur in assembly %s\n%s"
                        fwd dpp.fwd.tail dpp.fwd.body assembly.name (assemblySeq.arr |> format60)

                let rev = dpp.rev.Primer.RevComp()

                if not (assemblySeq.Contains(rev)) then
                    failwithf "rev primer validation failure.  Primer %O\ntail=%O\nbody=%O\n does not occur in assembly %s\n%s"
                        rev dpp.rev.tail dpp.rev.body assembly.name (assemblySeq.arr |> format60)
        ...
        for primer in pList do
            ...
            | DPP (dpp) ->
                ...
                let rev =
                    dpp.rev.body |> lastN 10 |> fun d -> d.RevComp()
                ...
                if not (ff || fr) then
                    failwithf "fwd XXX primer validation failure.  Primerlast10 %O\ntail=%O\nbody=%O\n does not occur fwd or rc in template %s\n>template\n%s\n>assembly\n%s"
                        fwd dpp.fwd.tail dpp.fwd.body assembly.name (templateSeq.arr |> format60)
                        (assemblySeq.arr |> format60)

                if not (rf || rr) then
                    failwithf "rev primer validation failure.  Primer %O\ntail=%O\nbody=%O\n does not occur fwd or rc in template %s\n>template\n%s\n>assembly\n%s"
                        rev dpp.rev.tail dpp.rev.body assembly.name (templateSeq.arr |> format60)
                        (assemblySeq.arr |> format60)
            | GAP
            | SANDWICHGAP -> ()
