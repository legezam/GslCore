...
type LexParseError =
    | LexParseError of context: GslParseErrorContext * stackTrace: StackTrace * node: AstNode
    | LexParseException of ex: exn * node: AstNode

module LexParseError =
    ...
    let toAstMessage: LexParseError -> AstMessage =
        function
        | LexParseError (context, stackTrace, errNode) ->
...
module LexAndParse =
    ...
    let private createErrorNode (inBuffer: LexBuffer<_>): AstNode =
        ...
        let pos = { Start = shiftedPos; End = shiftedPos }
        AstNode.ParseError({ Value = ""; Positions = [ pos ] })

    /// If exn is a GslParseError, format it.
    /// Otherwise, return None.
    let private parseExceptionToError (errNode: AstNode) exn =
        match exn with
        | GslParseError context -> LexParseError(context, StackTrace(exn), errNode)
        | _ -> LexParseException(exn, errNode)

    let lexAndParse (verbose: bool) (source: GslSourceCode): GslResult<AstTreeHead, LexParseError> =
        let inBuffer = LexBuffer<_>.FromString source.String

        let convertException exn =
            let errNode = createErrorNode inBuffer
            parseExceptionToError errNode exn
