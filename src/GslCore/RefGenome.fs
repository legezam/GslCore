...
type GenomeDef(libDir: string, name: string) as this =
    class
        ...
        member x.EnvLenLookup name defaultValue =
            if x.Env.ContainsKey(name) then (x.Env.[name] |> int) * 1<OneOffset> else 1<OneOffset> * defaultValue
        ...
        member x.Load() =
            ...
            feats <- Some(loadFeatures featsPath)

            let i1 =
                feats.Value
                |> Array.mapi (fun i f -> f.sysName, i)

            let i2 =
                feats.Value |> Array.mapi (fun i f -> f.gene, i)
        ...
        member x.GetAllFeats() =
            ...
            let distinctIndices =
                [ for kv in featIndex.Value -> kv.Value ]
                |> List.distinct
        ...
        member x.SuffixTree =
            ...
            | Some (st) -> st
            | None ->
                if (File.Exists(suffixTreePath.Value)) then
...
ERROR: location:
   ...
   at sgdrefformat.GenomeDef.get_SuffixTree() in C:\seq\GSL\gslc\sgdrefformat.fs:line 99
                    ...
                    failwithf "ERROR: loading suffix tree, unable to find file %s" suffixTreePath.Value
        ...
        interface IDisposable with
            member x.Dispose() =
                match suffixTree with
                | None -> ()
                | Some (std) -> (std :> IDisposable).Dispose()
